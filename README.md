...
**Pi Little Eye** is a security camera application for use with the Raspberry Pi and the official camera modules.<br>

- Camera viewable on all devices with a browser
- No mobile app required. All phone types and devices supported.
- No cloud. No subscriptions. No Telemetry.
- Image can be rotated and time stamped
- Video can be streamed to other devices e.g. for storage on a NAS
- Supports multiple user accounts and multiple simultaneous login sessions
- Supports IP whitelists and block lists
- Connection runs securely over https
- Auto-generates certificates. No SSL configuration necessary.
- Maintains complete access logs
...
If at any stage you forget the admin password, the camera configuration can be reset with the ```--factory-reset``` command line option.
...
There is a hamburger menu in the upper left corner that displays the options and a logout button top-right.
...
An admin user can lock (or unlock) another user's account. An admin can also logout another user. This can be useful if you have forgotten to logout on a device that you no longer have access to. If you wish to logout the current user, use the button in the top right. This will logout all sessions on all computers.
...
> Note: if the camera is accessed via a reverse proxy, all IPs on the route to the camera must be whitelisted for access to be granted including those used by the proxy server. Check the camera logs to see all the IP addresses that are visible to it. It also advisable to whitelist private IP ranges local to your broadband router such that the camera is accessible on your local wi-fi network. IPv6 is also supported.
...
Blocking IPs is not a very robust method of preventing access to the camera, someone intent on access might use a VPN or such to acquire a different IP. It can however be a useful counter-measure against nuisance bots that are persistently trying passwords. Often in practice bots will only be present on one or a limited range of IPs that can be blocked and the bot owner will not bother to move it but just go elsewhere. Another alternative to dealing with this kind of problem is to move the camera to a different port with the ```--port``` command line option.

<img src="docs/ip-ranges.png" width="400px" >

If you accidentally lock yourself out through misconfiguration of IP ranges, use the ```--disable-ip-lists``` command line option which will disable access lists and grant access to the camera to any IP.
...
To stream video from the camera to another device, an application key and secret are required to authenticate. An option to generate application keys can be found in the account settings under the configuration menu:
...
The video stream is available in MJPEG format at the endpoint: ```/api/v1/video/mjpeg```. The appkey and associated secret are supplied as GET parameters as follows. This will save the video to the file "output.mp4" in the native MPJEG codec:
...
When you connect to your bank over the web, how do you know that your bank actually owns the server you are attempting to connect to? This is achieved by having a trusted 3rd party counter-sign that the certificate presented by the bank's server does in fact belong to the bank. In this case we have not done that step because it involves either money or a beaurocratic verification process. Instead the camera auto-generates what's called a "self-signed" certificate which does not bear the counter-signatory from the 3rd party that your browser expects. While you definitely shouldn't click through the message if you are trying to connect to your bank (and that's why the message is so loud and bold) in this case it's fine because we already know who owns the camera - you do. In this instance there is no concern about who owns the device.

> One caveat is, if possible, at the point when you initially click through the certificate warning message, you should do so when connected to your camera locally via a network (router) that you personally own or otherwise trust to ensure you have indeed directly connected to your camera and not been sent sent elsewhere without your knowledge. You can then subsequently use the camera remotely.

If you already have a browser valid certificate for your raspberry pi, you can supply it on the command line and you won't get the warning message. However, if you're only ever going to be using the camera yourself, there is little point in going to that trouble as it won't do anything to enhance security over a self-signed certificate. All it does is get rid of the warning. Once you've clicked through it once, the browser will remember your decision and you won't be asked again.
